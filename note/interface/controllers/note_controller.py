...
@router.post("", status_code=201, response_model=NoteReponse)
@inject
def create_note(
...
@router.get("/{id}", response_model=NoteReponse)
@inject
def get_note(
    id: str,
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
    ...
    note = note_service.get_note(current_user.id, id)
...
@router.put("/{id}", response_model=NoteReponse)
@inject
def update_note(
    id: str,
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
