...
class NoteRepository(INoteRepository):
    def get_notes(self, user_id: str, page: int, items_per_page: int) -> tuple[int, list[NoteVO]]:
        with SessionLocal() as db:
            query = (
                db.query(Note).options(joinedload(Note.tags)).filter(Note.user_id == user_id)
            )
    ...
    def get_notes_by_tag_name(self, user_id: str, tag_name: str, page: int, items_per_page: int) -> tuple[int, list[NoteVO]]:
        with SessionLocal() as db:
            tag = db.query(Tag).filter_by(Tag.name == tag_name).first()
            if not tag:
                return 0, []
            ...
            query = (
                ...
                .filter(Note.user_id == user_id, Note.tags.any(id=tag.id))
            )   
